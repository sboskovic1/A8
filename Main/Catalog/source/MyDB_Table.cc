...
MyDB_TablePtr MyDB_Table :: alias (string toMe) {

	MyDB_TablePtr ret = make_shared <MyDB_Table> ();
	*ret = *this;	
	ret->mySchema = make_shared <MyDB_Schema> ();
	*(ret->mySchema) = *(this->mySchema);
	for (auto &a : ret->mySchema->getAtts ()) {
		a = make_pair (toMe + "_" + a.first, a.second);
	}
	return ret;
}
...
std::ostream& operator<<(std::ostream& os, const MyDB_TablePtr printMe) {
	if (printMe == nullptr) {
		...
		os << "name: " << printMe->tableName << "; " << printMe->last << "pages; file: " << printMe->storageLoc << "; schema: " << printMe->mySchema;
	}
    	return os;
}
